...
class NVIDIA(LLM):
    ...
    def __init__(self, **kwargs):
        ...
        if settings.llm.server_url:
            ...
            # self._chat_model = ChatNVIDIA(**self._default_params).mode(
            #     "nim", base_url=f"http://{settings.llm.server_url}/v1"
            # )
            if 'model' in self._default_params: del self._default_params['model']
            self._chat_model = ChatNVIDIA(base_url=f"http://{settings.llm.server_url}/v1",
